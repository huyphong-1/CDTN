...
from backtest_inventory_quarterly import build_quarterly_inventory_dataset, add_quarter_features
...
def main():
    ...
    if target not in ds.columns:
        raise ValueError("Missing QuarterEndInventory in dataset.")
    ...
    print("=== LINEAR REGRESSION (QuarterEndInventory) ===")
    print(
        f"n={report['n']} | MAE={report['MAE']:.2f} RMSE={report['RMSE']:.2f} "
        f"MAPE={report['MAPE']:.4f} R2={report['R2']:.4f}"
    )

    horizon = int(cfg.get("inventory", {}).get("forecast_quarters", 1))
    ...
    out_path = OUT_DIR / "inventory_forecast_linear_quarterly.csv"
    out.to_csv(out_path, index=False)

    (OUT_DIR / "inventory_linear_metrics.csv").write_text(
        "Metric,Value\n"
        f"n,{report['n']}\n"
        f"MAE,{report['MAE']}\n"
        f"RMSE,{report['RMSE']}\n"
    ...
    print(f"Saved: {OUT_DIR / 'inventory_linear_metrics.csv'}")
